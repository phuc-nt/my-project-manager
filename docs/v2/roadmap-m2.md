...
- **What shipped**:
  ...
  - **Linear gated WRITE**: `linear_createComment` only, allowlisted in `hard_block._MCP_ALLOWLIST["linear"]`, classified Lớp B (queued for approval). Destructive tools (`linear_delete*`, `linear_archive*`) hit Lớp A red line via `_DATA_LOSS_TOOL_MARKERS`. `src/actions/linear_write.py` + `linear` branch in `approved_dispatch.dispatch_approved_action`.
  - **D2 — Email/SMTP channel**: New `ActionGateway` mutation type `email_send` (added to `_MUTATING_TYPES`) routes all outbound email through gateway. ALL email = Lớp B (locked): `needs_interrupt` returns True; real send only via approved dispatch. `_hard_deny_email` (Lớp A) scans for secrets, rejects empty recipient/body.
  - **SmtpConfig** (`src/config/smtp_config.py`): host/user/from_addr/port/use_tls/recipients. Password ENV-ONLY (`SMTP_PASSWORD` at send-time), never config field. `src/actions/email_write.py` + `email_send` branch in `approved_dispatch`.
  - **Channel registry** (`src/agent/channel_registry.py`): `resolve_channels(config)` returns extra channels (email when SMTP configured; `()` otherwise). `deliver_extra_channels` gateway-routed; channel failure logged+skipped, never breaks core Slack+Confluence. Misconfigured SMTP (host set, no recipients) FAILS LOUD at config-build.
  - **Multi-graph wiring**: All 3 report graphs (report/okr/resource) wire email via `audience_delivery.deliver_extra_channels_and_summarize`. **Internal-only red line**: email skipped when `audience="external"` (email body is full report detail incl. per-assignee names/costs; external reports withhold that).
- **Files**: `src/config/smtp_config.py` (new), `src/actions/email_write.py` (new), `src/tools/linear_read.py` (new, reads), `src/actions/linear_write.py` (new, gated write), `src/agent/channel_registry.py` (new), `src/agent/audience_delivery.py` (refactored, channel delivery), hard_block/approved_dispatch (allowlist + dispatch branches).
- **Acceptance**: ✅ profile declares `integrations: { linear: { ... } }` + `smtp: { ... }` → Linear READ tools available + Linear createComment queued for approval → gate BLOCKS destructive Linear ops. Email queued for approval (Lớp B, all email) → gate OK. Misconfigured SMTP raises at config-build. No integrations + no SMTP ⇒ byte-identical pre-P11 (Slack+Confluence only).
- **Invariant preserved**: Every new write behind Action Gateway — Lớp A hard-deny + default-DENY allowlist + Lớp B approve. `classify()` / `needs_interrupt()` unchanged. Backward-compat: config flows through all 3 entry points (worker/cron/cli) automatically.

**Exit M3-P11**: ✅ ĐẠT. Integrations + multi-channel delivery shipped — 4 slices (config-driven MCP, Linear read, Linear gated write, Email channel). **RED LINE HELD**: all new writes (Linear comment, email) gate behind Action Gateway (Lớp A/B + audit + budget + dedup). Allowlist = deny-by-default; new tools must be explicitly added. Config flows through all 3 entry points. Backward-compat: no `integrations:` + no `smtp:` ⇒ byte-identical. **M3 MILESTONES (M3-P10, M3-P9, M3-P11) ✅ ALL SHIPPED** → agent orchestration foundation complete (skills + cross-agent memory + integrations).
...
- **What shipped**:
  ...
  - **B3 — Run replay / time-travel**: `src/runtime/replay.py`: `mpm agent replay <id> <thread> [--checkpoint <id>]`. Without `--checkpoint`, LISTS checkpoint history (structural summary, no PII); each row flagged `[replayable]`/`[needs-earlier-data]`. With `--checkpoint`, REPLAYS from frozen stored state (no re-fetch of live Jira/GitHub). **Safe-replay guard**: only checkpoints pending `deliver`/`approval_gate` or terminal are replayable; earlier nodes (perceive/analyze/compose) REFUSED — they have closure-local fetch boxes not checkpointed. Replay re-runs existing graph — any write re-enters same gateway Lớp A/B + dedup chain (no bypass). Time-travel state edits + re-fetch deferred.
  - **D3 — Workflow automation (READ-ONLY + PROPOSE)**: `src/automation/` package: `mpm agent automate <id> <automation.yaml> [--dry-run]`. Flat YAML with 3 step types (`read`/`analyze`/`propose`); single `when: field == value` condition; named-prompt `analyze` only. Engine chains WHITELISTED reads (jira.issues/github.prs/linear.issues/confluence.page, bypass gateway by design), runs analyze via LLM (prompts from `src/automation/prompts.py`), routes proposals through `ActionGateway.execute()` WITHOUT handler ⇒ Lớp B ENQUEUES, non-Lớp-B no-ops. **NEVER auto-executes** writes. Fail-closed schema (unknown step/tool/target/prompt → error; `when` is single `==`). Example: `docs/v2/examples/automation-blocker-stakeholder-note.yaml`.
- **Files**: `src/runtime/run_config.py` (invoke_config + tracing_env_on), `src/runtime/replay.py` (new, replay engine), new `src/automation/{schema,prompts,propose,engine}.py`, wire into all 3 entry points (worker/cron/cli).
- **Acceptance**: ✅ Tracing OFF (default) ⇒ byte-identical. Tracing ON + env set → tracer attaches, observer sees live LangSmith logs. Replay list → summary; replay checkpoint → re-run frozen state, gateway re-checks writes. Automation YAML → proposals queued at gateway (Lớp B), never auto-approved. Fail-closed: bad YAML → error.
- **THE INVARIANT (unchanged by P12)**:
  ...
  - Action-Gateway red line — Lớp A hard-deny + allowlist default-DENY + Lớp B approve, `classify()`/`needs_interrupt()` unchanged — is **untouched**.
  - Backward-compat: tracing OFF + no automation invoked ⇒ byte-identical pre-P12.

**Exit M3-P12 (FINAL M3)**: ✅ ĐẠT. Automation + observability shipped — 3 slices (B4 tracing, B3 replay, D3 automation). **RED LINE HELD**: all proposals (automation + replay writes) gate through Action Gateway (Lớp A/B unchanged). Tracing is observability-only, zero action-path touch. Backward-compat: tracing DEFAULT OFF (no perf delta), no automation invoked ⇒ byte-identical. **M3 COMPLETE** (all 4 milestones: P10 skills, P9 cross-agent memory, P11 integrations, P12 automation+observability).
