...
def _run_report() -> int:
    ...
    graph = build_report_graph(get_checkpointer())
    result = graph.invoke({}, config={"configurable": {"thread_id": "report"}})

    print(result.get("report_text", "(no report)"))
    cost = result.get("cost_usd")
    delivered = result.get("delivered")
    print(
        f"\n[delivered: {delivered} · {result.get('delivery_summary', '')} · "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args:
        print(
            'usage: python -m src.entrypoints.cli "your message" | report',
            file=sys.stderr,
        )
        return 2
